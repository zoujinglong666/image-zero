...
CREATE TABLE IF NOT EXISTS drawing_tasks (
    ...
    prompt          VARCHAR(2000)                                  COMMENT '提示词',
    negative_prompt VARCHAR(2000)                                 COMMENT '负面提示词',
    ...
    input_data      VARCHAR(2000)                                  COMMENT '输入数据JSON',
...
ALTER TABLE prompt_library
    ADD COLUMN content_cn  TEXT AFTER prompt_text,
    ADD COLUMN sort_order  INT NOT NULL DEFAULT 0 AFTER tags,
    ADD COLUMN status      VARCHAR(20) NOT NULL DEFAULT 'published' AFTER sort_order;